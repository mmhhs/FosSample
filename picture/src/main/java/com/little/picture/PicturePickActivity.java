...
import com.little.picture.adapter.NullPictureAdapter;
...
//        FrescoUtils.init(this);
     ...
     * @param size
     */
    public static void startAction(Activity activity,int pickType,int size) {
        Intent intent = new Intent(activity, PicturePickActivity.class);
        intent.putExtra(PicturePickActivity.PICTURE_PICK_IMAGE, size);
...
//                new String[] { "image/jpeg", "image/png" }, MediaStore.Images.Media.DATE_MODIFIED+ " desc");不支持PNG
    ...
    private void setFolderShow(int position) {
        ...
        if(folderImageEntityList!=null&&folderImageEntityList.size()>0) {
            ...
            //没有图片的情况下，单独增加拍照按钮
            NullPictureAdapter nullPictureAdapter = new NullPictureAdapter(this);
            gridView.setAdapter(nullPictureAdapter);
            nullPictureAdapter.setOnItemClickListener(new IOnItemClickListener() {
                @Override
                public void onItemClick(int position) {
                    //空图片拍照点击事件
